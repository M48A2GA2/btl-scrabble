...
void Game::handleMouseRelease(int x, int y)
{
    if (isDragging && selectedTileIndex != -1)
    {
        ...
        if (getBoardPosition(x, y, row, col))
        {
            ...
            {
                // Start new word - check if valid position
                ...
                {
                    std::cout << "Cannot place tile at that position" << std::endl;
                }
            }
        }
    }
...
void Game::renderGameInfo()
{
    ...
    // Game title
    textRenderer.renderText("SCRABBLE GAME", infoX, infoY - 30, {255, 255, 255, 255});

    // Scores
    textRenderer.renderText("SCORES:", infoX, infoY, {255, 255, 0, 255});
    for (size_t i = 0; i < players.size(); i++)
    {
        std::string scoreText = players[i].getName() + ": " + std::to_string(players[i].getScore());
        SDL_Color color = (i == currentPlayer) ? SDL_Color{0, 255, 0, 255} : SDL_Color{255, 255, 255, 255};
        textRenderer.renderText(scoreText, infoX, infoY + 25 + i * 25, color);
    }

    // Current player info
    int currentPlayerY = infoY + 100;
    std::string currentPlayerText = "Current Player:";
    textRenderer.renderText(currentPlayerText, infoX, currentPlayerY, {255, 255, 0, 255});
    textRenderer.renderText(players[currentPlayer].getName(), infoX, currentPlayerY + 25, {0, 255, 0, 255});

    // Game status
    int statusY = currentPlayerY + 70;
    std::string statusText;
    if (isPlacingWord)
    {
        statusText = "Placing word...";
        textRenderer.renderText(statusText, infoX, statusY, {255, 255, 0, 255});
        textRenderer.renderText("ENTER: Commit", infoX, statusY + 25, {200, 200, 200, 255});
        textRenderer.renderText("ESC: Cancel", infoX, statusY + 45, {200, 200, 200, 255});
    }
    else
    {
        statusText = "Click tile to place";
        textRenderer.renderText(statusText, infoX, statusY, {255, 255, 255, 255});
        textRenderer.renderText("SPACE: Skip turn", infoX, statusY + 25, {200, 200, 200, 255});
    }

    // Tiles remaining
    int tilesY = statusY + 100;
    std::string tilesText = "Tiles remaining: " + std::to_string(tileBag.remainingTiles());
    textRenderer.renderText(tilesText, infoX, tilesY, {255, 255, 255, 255});

    // Game instructions
    int instructY = tilesY + 50;
    textRenderer.renderText("INSTRUCTIONS:", infoX, instructY, {255, 255, 0, 255});
    std::vector<std::string> instructions = {
        "1. Drag tiles to board",
        "2. Form valid words",
        "3. Connect to existing tiles",
    ...
    {
        textRenderer.renderText(instructions[i], infoX, instructY + 25 + i * 20, {200, 200, 200, 255});
    }
}
...
void Game::commitWord()
{
    ...
    // Calculate and add score using the improved scoring system
    int totalScore = calculateCompleteWordScore(pendingPlacements);
    
    auto words = getFormedWords();
    std::cout << "Words formed:" << std::endl;
    ...
    std::cout << "Total score added: " << totalScore << std::endl;
...
bool Game::validateWordPlacement() const
{
    ...
    // Check for gaps in word placement
    if (hasGapsInWordPlacement())
    {
        std::cout << "Invalid placement: gaps found in word" << std::endl;
        return false;
    }

    // Validate adjacency and connectivity rules
    if (board.isFirstMovePlayed())
    {
        // After first move, new tiles must connect to existing tiles
        bool hasConnection = false;
        for (const auto &pos : pendingPlacements)
        {
            if (board.isAdjacentToExistingTile(pos.first, pos.second))
            {
                hasConnection = true;
        ...
        if (!hasConnection)
        {
            std::cout << "Invalid placement: must connect to existing tiles" << std::endl;
            return false;
        }
    }
    else
    {
        ...
        if (!includesCenter)
        {
            std::cout << "Invalid placement: first move must include center square" << std::endl;
            return false;
        }
    }

    // Get all formed words and validate against dictionary
    auto words = getFormedWords();
    for (const auto &word : words)
    {
        if (word.length() < 2)
